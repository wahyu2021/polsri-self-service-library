    ...
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
            (position) => {
                ...
                // Update UI
                gpsText.innerText = "Lokasi Terkunci (Akurasi: " + Math.round(position.coords.accuracy) + "m)";
...
async function processCheckIn(qrValue) {
    if (!currentLat) {
        showFeedback(false, "GPS Error", "Lokasi belum ditemukan. Pastikan GPS aktif.");
    ...
    statusDisplay.className = "mb-4 p-3 rounded-xl text-center text-sm font-bold bg-slate-100 text-slate-500";
