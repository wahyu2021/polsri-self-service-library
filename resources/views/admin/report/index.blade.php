<x-layouts.admin title="Laporan & Denda">
    ...
    <div class="hidden print:block mb-8 text-black font-serif">
        <div class="flex items-center border-b-2 border-black pb-4 mb-4 gap-4">
            ...
            <div class="flex-1 text-center">
                <h2 class="text-sm uppercase font-bold tracking-wide">KEMENTERIAN PENDIDIKAN, KEBUDAYAAN, RISET, DAN TEKNOLOGI</h2>
        ...
        <div class="text-center mb-6">
            ...
            <p class="text-sm">Periode: {{ \Carbon\Carbon::parse($startDate)->isoFormat('D MMMM Y') }} s.d. {{ \Carbon\Carbon::parse($endDate)->isoFormat('D MMMM Y') }}</p>
        </div>
    </div>
    ...
    <div class="flex flex-col lg:flex-row lg:items-end justify-between gap-6 mb-8 print:hidden">
        ...
        <form action="{{ route('admin.reports.index') }}" method="GET" class="bg-white p-2 rounded-xl border border-slate-200 shadow-sm flex flex-col sm:flex-row items-center gap-2">
            ...
            <button type="submit" formaction="{{ route('admin.reports.export') }}" class="px-4 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-bold rounded-xl shadow-lg shadow-emerald-500/20 transition-all hover:-translate-y-0.5">
                Export CSV
            </button>
        </form>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 print:grid-cols-3 print:gap-4">
        <!-- Revenue -->
        <div class="bg-white p-6 rounded-2xl border border-slate-100 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] print:border-black print:border print:rounded-none">
            <p class="text-slate-500 text-sm font-medium print:text-black">Total Pendapatan Denda</p>
            <h3 class="text-3xl font-bold text-emerald-600 mt-2 print:text-black">Rp {{ number_format($totalRevenue, 0, ',', '.') }}</h3>
            <p class="text-xs text-emerald-600/80 mt-1 font-medium bg-emerald-50 inline-block px-2 py-0.5 rounded print:hidden">
        ...
        <div class="bg-white p-6 rounded-2xl border border-slate-100 shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] print:border-black print:border print:rounded-none">
            ...
            <h3 class="text-3xl font-bold text-slate-900 mt-2 print:text-black">Rp {{ number_format($averageFine, 0, ',', '.') }}</h3>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 print:block">
        ...
        <!-- Detailed Logs (2/3) -->
        <div class="lg:col-span-2 bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden print:w-full print:border-black print:border print:rounded-none print:shadow-none mb-8">
            <div class="p-6 border-b border-slate-50 flex items-center justify-between print:border-black print:p-4">
                ...
                <button onclick="window.print()" class="text-xs font-bold text-slate-500 hover:text-slate-900 flex items-center gap-1 print:hidden">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm text-left">
                    <thead class="bg-slate-50 text-slate-500 font-medium border-b border-slate-100 print:bg-white print:text-black print:border-black">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-right print:px-2 print:py-2 print:border print:border-black">Nominal</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-50 print:divide-black">
                        ...
                        <tr class="hover:bg-slate-50/50 transition print:hover:bg-transparent">
                            <td class="px-6 py-4 text-slate-500 text-xs print:px-2 print:py-2 print:border print:border-black print:text-black">
                            ...
                            <td class="px-6 py-4 print:px-2 print:py-2 print:border print:border-black">
                                <div class="font-medium text-slate-900 print:text-black">{{ $fine->user->name }}</div>
                                <div class="text-xs text-slate-500 print:text-black">{{ $fine->user->nim }}</div>
                            </td>
                            <td class="px-6 py-4 print:px-2 print:py-2 print:border print:border-black">
                                <div class="text-slate-700 truncate max-w-[150px] print:text-black print:whitespace-normal">{{ $fine->book->title }}</div>
                            </td>
                            <td class="px-6 py-4 text-right font-bold text-slate-900 print:px-2 print:py-2 print:border print:border-black print:text-black">
                                Rp {{ number_format($fine->fine_amount, 0, ',', '.') }}
                            </td>
                        </tr>
                        @empty
                        <tr>
                            <td colspan="4" class="px-6 py-12 text-center text-slate-400 print:border print:border-black print:text-black">
        ...
        <!-- Top Violators (1/3) -->
        <div class="lg:col-span-1 bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden h-fit print:hidden">
    ...
    <div class="hidden print:flex justify-end mt-12 text-black font-serif page-break-inside-avoid">
        <div class="text-center min-w-[200px]">
            <p class="mb-1">Palembang, {{ now()->isoFormat('D MMMM Y') }}</p>
            <p class="font-bold mb-20">Kepala UPT Perpustakaan</p>
